...
<script>
import { getCommentsList, updateComments, deleteComments } from '@/api/admin'
...
export default {
  ...
  methods: {
    handleDeleteBatch () {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          deleteComments({ ids: arr }).then((res) => {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          if (settings.forbit !== '1') {
            ...
            updateComments({ ids: arr, settings: { status: '1' } }).then(
              (res) => {
                // this.tableData.splice(index, 1)
    ...
    handleRowRemove (row, index) {
      this.$Modal.confirm({
        title: '确定删除吗？',
        content: `删除${row.name}的评论`,
        onOk: () => {
          deleteComments({ ids: row._id }).then((res) => {
            this.tableData.splice(index, 1)
            this.$Message.success('删除成功！')
          })
        },
