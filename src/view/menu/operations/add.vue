    ...
    <Modal v-model="showStatus" title="添加资源信息" @on-ok="ok" @on-cancel="cancel" :loading="loading">
      <Form :model="localItem" :label-width="80" :rules="ruleValidate" ref="table">
        <FormItem label="资源名称" prop="name">
          <i-input v-model="localItem.name" placeholder="请输入资源名称"></i-input>
        </FormItem>
        <FormItem label="资源路径" prop="path">
          <i-input v-model="localItem.path" placeholder="请输入资源路径"></i-input>
        </FormItem>
        ...
        <FormItem label="资源描述">
          <i-input type="textarea" v-model="localItem.regmark" placeholder="请输入资源描述"></i-input>
        </FormItem>
      </Form>
    </Modal>
  </div>
