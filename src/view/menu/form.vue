  ...
  <div>
    <Form :disabled="!isEdit" ref="form" :model="formDataItem" :rules="formRules" :label-width="80">
      <FormItem label="菜单标题" prop="title">
        <i-input v-model="formDataItem.title" placeholder="请输入菜单名称"></i-input>
      </FormItem>
      ...
      <template v-if="formDataItem.type !== 'link'">
        <FormItem label="组件名称" prop="name">
          <i-input v-model="formDataItem.name" placeholder="请输入组件名称"></i-input>
        </FormItem>
        <FormItem label="组件">
      ...
      <template v-else>
        <FormItem label="链接">
          <i-input v-model="formDataItem.link" placeholder="请输入跳转链接"></i-input>
        </FormItem>
      </template>
